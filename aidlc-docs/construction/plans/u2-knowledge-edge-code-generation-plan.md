...
- [x] 5.1 `expand()` — `safe_resolve()` + **경로 탈출 거부**
- [x] 5.2 `knowledge_scope` glob 매치 → `ScopeViolationError` (BR-S-03)
- [x] 5.3 파일 종류 판정 (BR-S-09 매핑 테이블)
- [x] 5.4 프런트매터 파싱 → `display_title`, `as_of`, `formality`
- [x] 5.5 스크립트·설정(`.py`/`.yaml`)은 주석 헤더 파싱
- [x] 5.6 **`run_log`는 마지막 200줄**, 그 외는 앞부분. 256KB 상한 (BR-S-10)
- [x] 5.7 `Chunk` 생성 — **`tier`는 채우지 않는다** (U1의 일)
- [x] 5.8 `tests/unit/test_store_read.py` — 경로 탈출, scope 위반, 종류별 파싱

## Step 6 · `select_paths()` — 경로 선택

- [x] 6.1 프롬프트 구성 — 경로 + `display_title` + 세션 `focus`/`summary`
- [x] 6.2 **파일 본문을 넣지 않는다** 🔴 (BR-S-02)
- [x] 6.3 출력은 **인덱스 배열** (`{"selected":[0,1]}`) — 경로 문자열 생성 금지
- [x] 6.4 실패/파싱 오류 → `open_paths` 전체 반환
- [x] 6.5 `tests/unit/test_select_paths.py` — 본문 미포함 확인, 실패 폴백

**6.2 검증**: 프롬프트 문자열에 `Chunk.text`가 등장하지 않음을 테스트로 확인.

## Step 7 · `list_agents()` — 지목 목록

- [x] 7.1 `AgentCard` 구성 — `expertise`는 항상, 나머지는 `disclose` 반영
- [x] 7.2 `activity_status` 매핑 (LIVE→active / STALE→away / EXPIRED→offline)
- [x] 7.3 `question_count_today` — `audit` + `local_queries`에서 집계
- [x] 7.4 **`current_focus_summary` — Gatekeeper로 식별자 제거 요약** 🔴 (BR-S-06)
- [x] 7.5 **요약 실패 시 `None`. 원문 폴백 없음** (fail closed)
- [x] 7.6 요약 캐시 (키: `entity_id` + `updated_at`, TTL 5분)
- [x] 7.7 `disclose: false`인 필드는 `None`
- [x] 7.8 앱 시작 시 워밍업 (첫 화면에서 3초 대기 방지)
- [x] 7.9 `tests/unit/test_list_agents.py`

## Step 8 · PBT

- [x] 8.1 `tests/generators.py`에 `sessions()`, `verified_qas()` 추가
- [x] 8.2 PB-S1: `expand()`가 root 밖 경로에 항상 `PathEscapeError`
- [x] 8.3 PB-S2: `Session` 직렬화 왕복 항등
- [x] 8.4 PB-S3: `freshness()` 단조 악화
- [x] 8.5 **PB-S4: `list_agents()` 결과에 `Session.focus`/`summary` 문자열 부재** 🔴
- [x] 8.6 PB-S5: `read()` 결과의 모든 `internal_path`가 `knowledge_scope`에 매치

**8.5가 FR-31의 검증이다.** 고객사명·금액·인명을 포함한 임의 세션을 생성해 목록 응답 JSON에 원문이 없음을 확인한다.

## Step 9 · 경계 확인

- [x] 9.1 `grep -c "boto3\|BrokerClient\|bedrock" src/mesh/store.py` == 0
- [x] 9.2 `rglob("**/*")` 같은 전역 스캔이 없음 확인 (BR-S-01)
- [x] 9.3 import 경계 테스트 통과
- [x] 9.4 커밋
...
- [x] 세션 3개 로드 + `verified_qa` 병합 (`tier` 보존)
- [x] `${MESH_DATA_ROOT}` 치환 + 경로 탈출 거부 🔴
- [x] `knowledge_scope` 위반 거부 (에이전트 간 지식 격리)
- [x] 신선도 3단 판정 (시각 조작 테스트)
- [x] **데몬 없이 세션 JSON만으로 3막 통과** (FR-21)
- [x] `list_agents()` 응답에 고객사명·경로 부재 🔴
- [x] `select_paths()` 프롬프트에 파일 본문 부재 🔴
- [x] `run_log` 마지막 200줄 읽기
- [x] PB-S1~PB-S5 통과
- [x] `store.py`에 경계 밖 import 부재
- [x] 전역 파일 스캔 부재
